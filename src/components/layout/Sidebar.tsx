...
export function Sidebar() {
  const { chats, isSidebarOpen, setSidebarOpen, activeChatId, removeChat, togglePin, clearAllChats } = useChatStore();
  ...
  return (
    <>
      {/* Mobile overlay */}
      {mounted && !isDesktop && isSidebarOpen && (
        <div 
      ...
      {/* Sidebar Content */}
      <aside 
        className={`h-full bg-[#FAFAFA] dark:bg-[#1A1918] border-r border-border dark:border-[#33312E] transition-all duration-300 ease-in-out z-50 fixed md:relative flex flex-col ${
          isSidebarOpen ? "w-[280px]" : "w-0 md:w-[68px] overflow-hidden"
        }`}
      >
        <div className="flex flex-col h-full w-full overflow-hidden">
          
          <div className={`flex items-center ${!isSidebarOpen ? 'justify-center' : 'justify-between'} px-4 py-3 min-h-[52px]`}>
            {isSidebarOpen && !isSearchActive && <Link href="/" className="font-serif text-[17px] tracking-wide font-medium text-foreground dark:text-[#E6E4DF] whitespace-nowrap">Drasa AI</Link>}
          ...
          <div className="flex-1 overflow-y-auto px-3 space-y-6 pb-20 custom-scrollbar mt-2">
              ...
              <div className="space-y-0.5">
                {filteredChats.length === 0 ? (
                  ...
                  <>
                    ...
                    {isSidebarOpen && (
                      <div className="pt-2 mt-2 border-t border-border/30 dark:border-[#33312E]/30">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-medium text-foreground">
                            {userData?.usage?.messagesUsedToday || 0} / {
                              (userData?.plan === 'ultimate' || userData?.plan === 'premium') ? 500 : 
                              userData?.plan === 'pro' ? 150 : 
                              userData?.plan === 'starter' ? 50 : 20
                            }
                          </span>
                        </div>
                        <div className="h-1.5 w-full bg-muted dark:bg-[#1A1918] rounded-full overflow-hidden">
                          <div className="h-full bg-primary transition-all" style={{ width: `${Math.min(100, ((userData?.usage?.messagesUsedToday || 0) / (
                            (userData?.plan === 'ultimate' || userData?.plan === 'premium') ? 500 : 
                            userData?.plan === 'pro' ? 150 : 
                            userData?.plan === 'starter' ? 50 : 20
                          )) * 100)}%` }} />
...
function RecentItem({ chat, isActive, collapsed, onPin, onDelete }: { chat: any, isActive?: boolean, collapsed?: boolean, onPin?: (e: any) => void, onDelete?: (e: any) => void }) {
  return (
    <Link href={`/c/${chat.id}`} title={collapsed ? chat.title : undefined}>
      <div className={`w-full flex items-center justify-between group ${collapsed ? 'justify-center px-0' : 'px-3'} py-2 rounded-lg transition-all duration-200 text-[13px] text-left cursor-pointer border ${
        isActive 
        ...
        <div className="flex items-center gap-2 overflow-hidden truncate">
          {collapsed ? <MessageSquare size={16} className={isActive ? "text-foreground dark:text-[#E6E4DF]" : ""} /> : <span className="truncate">{chat.title}</span>}
        </div>
        {!collapsed && (
