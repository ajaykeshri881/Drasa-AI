  ...
  return (
    <div
      className={cn(
        ...
        isUser ? "bg-transparent" : "bg-muted/30 dark:bg-[#1A1918]"
      )}
    >
          ...
          <div className="space-y-3 mt-2">
            {toolInvocations.map((tool: any) => {
                  ...
                  <div key={tool.toolCallId} className="bg-card dark:bg-[#2A2928] border border-border dark:border-[#33312E] rounded-xl p-4 max-w-sm flex flex-col gap-3 shadow-sm">
                    ...
                    {!isGenerating && tool.args && 'html' in tool.args && onViewArtifact && (
                      <button 
                        ...
                        className="w-full mt-1 py-2 px-3 bg-primary/10 hover:bg-primary/20 text-primary dark:text-[#C36A4F] dark:bg-[#C36A4F]/10 dark:hover:bg-[#C36A4F]/20 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
                      >
              ...
              if (tool.toolName === "internet_search") {
                ...
                return (
                  <div key={tool.toolCallId} className="bg-card dark:bg-[#2A2928] border border-border dark:border-[#33312E] rounded-xl p-4 max-w-sm flex items-center gap-4 shadow-sm">
                    <div className="p-2.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-lg flex-shrink-0">
                      {isSearching ? <Loader2 size={24} className="animate-spin" /> : <Search size={24} />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-sm font-semibold text-foreground dark:text-white truncate">Web Search</h4>
                      <p className="text-xs text-muted-foreground dark:text-[#8A8985] truncate">
                        {isSearching ? `Searching for "${args?.query || '...'}"` : `Searched for "${args?.query || '...'}"`}
                      </p>
                    </div>
                  </div>
                );
