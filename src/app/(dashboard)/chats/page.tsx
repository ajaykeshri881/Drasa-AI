            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-9 pr-4 py-2 bg-card dark:bg-[#2A2928] border border-border dark:border-[#33312E] rounded-lg text-sm w-[240px] focus:outline-none focus:border-primary dark:focus:border-[#C36A4F] transition-colors"
                />
              </div>
              <button 
            ...
            <div className="grid gap-3">
              {filteredChats.map(chat => (
                <Link 
                  key={chat.id} 
                  href={`/c/${chat.id}`}
                  className="group flex items-center justify-between p-4 bg-card dark:bg-[#2A2928] border border-border dark:border-[#33312E] hover:border-border/80 dark:hover:border-[#4A4946] rounded-xl transition-all duration-200 hover:shadow-sm"
                >
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-10 h-10 rounded-full bg-primary/10 dark:bg-[#C36A4F]/10 flex items-center justify-center flex-shrink-0 text-primary dark:text-[#C36A4F]">
                      <MessageSquare size={20} />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-medium text-foreground dark:text-[#E6E4DF] truncate text-[15px] mb-1 group-hover:text-primary dark:group-hover:text-[#C36A4F] transition-colors">
                        {chat.title}
                      </h3>
                      <div className="flex items-center gap-3 text-xs text-muted-foreground dark:text-[#8A8985]">
                        <span className="flex items-center gap-1">
                          <Calendar size={12} />
                          {new Date(chat.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                        </span>
                        {chat.mode && (
                          <span className="px-2 py-0.5 rounded-md bg-muted dark:bg-[#33312E] capitalize">
                            {chat.mode}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      ...
                      className="p-2 text-muted-foreground hover:text-destructive dark:text-[#8A8985] dark:hover:text-red-400 hover:bg-destructive/10 dark:hover:bg-red-900/20 rounded-lg transition-colors"
                      title="Delete chat"
                    >
                      <Trash2 size={16} />
                    </button>
                    <div className="p-2 text-muted-foreground dark:text-[#8A8985]">
