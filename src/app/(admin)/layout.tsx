...
import { LayoutDashboard, Users, Settings, AlertTriangle, ChevronLeft, CreditCard, Server, Megaphone } from "lucide-react";
    ...
    <div className="flex h-screen bg-background dark:bg-[#1A1918] text-foreground dark:text-[#E6E4DF] font-sans selection:bg-primary/30">
      ...
      <aside className="w-[260px] flex-shrink-0 border-r border-border dark:border-[#33312E] bg-card dark:bg-[#1F1E1D] flex flex-col h-full">
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          <NavItem href="/users" icon={<Users size={18} />} label="Users" />
          <NavItem href="/payments" icon={<CreditCard size={18} />} label="Payments" />
          <NavItem href="/queues" icon={<Server size={18} />} label="Queues" />
          <NavItem href="/models" icon={<Settings size={18} />} label="Models" />
          <NavItem href="/sponsors" icon={<Megaphone size={18} />} label="Sponsors" />
          <NavItem href="/alerts" icon={<AlertTriangle size={18} />} label="Broadcasts" />
        </nav>
